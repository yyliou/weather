...
test_that("nearest-id pool returns the closest stations to each target", {
  targets <- data.frame(region = c("L", "R"), lon = c(0, 10), lat = c(0, 0),
                        stringsAsFactors = FALSE)
  stations <- data.frame(station_id = c("A", "B", "C"),
                         lon = c(0.1, 5, 10.1), lat = c(0, 0, 0),
                         stringsAsFactors = FALSE)
  pools <- .tww_nearest_ids(targets, stations, pool_size = 1)
  expect_equal(pools[[1]], "A")     # nearest to (0,0)
  expect_equal(pools[[2]], "C")     # nearest to (10,0)
})
